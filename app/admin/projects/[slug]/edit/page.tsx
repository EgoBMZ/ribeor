...
import { getProjectBySlug, updateProject } from "../../../../../lib/firebase/firestore";
...
export default function EditProjectPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!title || !description || !content || !user) return;
    
    setIsSubmitting(true);
    try {
      const techArray = technologies.split(",").map(t => t.trim()).filter(Boolean);
      
      await updateProject(currentSlug, {
        title,
        description,
        content,
        repoUrl,
  ...
  return (
    <div className="w-full flex flex-col gap-6">
      <h1 className="text-3xl font-oswald text-text-primary uppercase tracking-tight">Editar Proyecto</h1>
        ...
        {(activeTab === "edit" || activeTab === "split") && (
          <form onSubmit={handleSubmit} className={`flex flex-col gap-6 w-full ${activeTab === "split" ? "xl:w-1/2" : ""}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex flex-col gap-2">
                <label className="text-sm uppercase tracking-wider font-bold text-text-meta">Slug (URL) - No editable</label>
                <input 
                  type="text" 
                  value={currentSlug} 
                  disabled
                  className="p-3 rounded-lg bg-bg-secondary border border-border-color text-text-meta outline-none opacity-60 cursor-not-allowed font-mono text-sm"
                />
              </div>
            </div>
